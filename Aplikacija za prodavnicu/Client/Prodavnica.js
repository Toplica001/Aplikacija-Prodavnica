...
export class Prodavnica{
    ...
    iscrtajProdavnicu(){
        ...
        prikaziBtn.onclick=(ev)=>this.prikaziPodatke(tb); //tb je tbody tabele
    ...
    prikaziPodatke(host){ //host je tbody tabele gde ce da crta proizvode
        host.innerHTML="";
        ...
        if(od==="" && doo===""){ //ako samo po tipu pretrazuje
            ...
            fetch("https://localhost:5001/Prodavnica/PreuzmiFiltrirano/"+this.id+"/"+tip+"/"+od+"/"+doo,{
                ...
                data.json().then(proizvodi=>{
                    ...
                    this.Proizvodi.forEach(p=>{
                        p.iscrtajProizvod(host); //host je tbody
                    })
                });
            });
        }
